...
Paths (relative to repo root):
   SQL:  sql/sqlite/*.sql
   CSV:  data/raw/retail/store_bjean.csv
   CSV:  data/raw/retail/sale_bjean.csv
   DB:   artifacts/sqlite/retail_bjean.sqlite
...
STORE_CSV: Final[Path] = DATA_RAW_DIR / "store_bjean.csv"
SALE_CSV: Final[Path] = DATA_RAW_DIR / "sale_bjean.csv"
DB_PATH: Final[Path] = ARTIFACTS_DIR / "retail_bjean.sqlite"
...
def load_sale_csv(con: sqlite3.Connection, csv_path: Path) -> None:
    """Load sale_bjean.csv into the sale table."""
    ...
    with csv_path.open(mode="r", encoding="utf-8", newline="") as f:
        ...
        rows = []
        for r in reader:
            amount_raw = r["amount"].replace("$", "").replace(",", "").strip()
...
def main() -> None:
    ...
    try:
        ...
        run_sql_query(con, SQL_DIR / "bjean_retail_query_sales_count.sql")
        run_sql_query(con, SQL_DIR / "bjean_retail_query_sales_aggregate.sql")
        run_sql_query(con, SQL_DIR / "bjean_retail_query_sales_by_category.sql")

        # ----------------------------------------------------
        # STEP 4: RUN KPI QUERY (ACTION-DRIVEN)
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "bjean_retail_query_kpi_revenue.sql")
