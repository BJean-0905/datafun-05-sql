...
SELECT
  ...
  '$' || printf('%.2f', SUM(CAST(REPLACE(sa.amount, '$', '') AS REAL))) AS total_revenue,
  '$' || printf('%.2f', AVG(CAST(REPLACE(sa.amount, '$', '') AS REAL))) AS avg_sale_amount
FROM store AS s
JOIN sale AS sa
...
GROUP BY
  ...
  s.city,
  s.region
ORDER BY SUM(CAST(REPLACE(sa.amount, '$', '') AS REAL)) DESC;
